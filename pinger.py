...
def receiveOnePing(mySocket, ID, timeout, destAddr):
    ...
    while 1:
        ...
        type, code, checksum, pID, sq = struct.unpack("bbHHh", icmp_header)

        print("The header received in the icmp reply is: ", type, code, checksum, pID, sq)
        if pID == ID:
            bytesinDbl = struct.calcsize("d")
            print('bytesinDbl:', bytesinDbl)
            timeSent = struct.unpack("d", recPacket[20:20 + bytesinDbl])[0]
...
def ping(host, timeout=1):
    ...
    for i in range(0, 4):  # Four pings will be sent (loop runs for i=0, 1, 2, 3)
        delay, statistics = [doOnePing(dest, timeout),
                             doOnePing(dest, timeout)]  # what is stored into delay and statistics?
        datadict = {'bytes': 8, 'rtt': delay, 'ttl': timeout}
        response = response.append(datadict,ignore_index=True)  # store your bytes, rtt, and ttl here in your response pandas dataframe. An example is commented out below for vars
        print(delay)
